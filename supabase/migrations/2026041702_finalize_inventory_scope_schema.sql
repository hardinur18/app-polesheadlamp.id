...
alter table if exists public.products
  ...
  using (
    case
      when technician_id is null then null
      when technician_id::text ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
        then technician_id::text::uuid
      else null
    end
  );
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'products_technician_id_fkey'
      and conrelid = 'public.products'::regclass
  ) then
    alter table public.products
      add constraint products_technician_id_fkey
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'stock_transactions_technician_id_fkey'
      and conrelid = 'public.stock_transactions'::regclass
  ) then
    alter table public.stock_transactions
      add constraint stock_transactions_technician_id_fkey
      foreign key (technician_id) references public.profiles(id) on delete set null;
  end if;
end $$;
...
create index if not exists products_technician_id_idx
  on public.products(technician_id);
