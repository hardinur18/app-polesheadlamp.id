...
export const ui = {
  ...
  r: {
    control: 'rounded-md', // buttons, chips, list items
    field: 'rounded-lg', // inputs, cards, account selector
    pill: 'rounded-full', // avatars, count badges
  },
  // Sizing scale
  h: {
    header: 'h-16', // every column header band (64px)
    control: 'h-9', // primary controls / composer buttons
...
export const whatsAppInboxStyle = {
  ...
  '--wa-icon-btn': '36px',
  '--wa-icon-btn-sm': '32px',
  '--wa-account-select-h': '64px',
  '--wa-account-icon': '44px',
  '--wa-avatar-row': '44px',
...
export const cls = {
  layoutGrid:
    'grid min-h-0 flex-1 grid-cols-1 border-y border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950 lg:grid-cols-[var(--wa-list-w-compact)_minmax(0,1fr)] xl:grid-cols-[var(--wa-sidebar-w)_var(--wa-list-w)_minmax(0,1fr)]',
  layoutGridWithCustomer:
    'grid min-h-0 flex-1 grid-cols-1 border-y border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950 lg:grid-cols-[var(--wa-list-w-compact)_minmax(0,1fr)] xl:grid-cols-[var(--wa-sidebar-w)_var(--wa-list-w)_minmax(0,1fr)] min-[1900px]:grid-cols-[var(--wa-sidebar-w)_var(--wa-list-w)_minmax(0,1fr)_var(--wa-detail-w)]',
  headerBand:
  ...
  sectionToggle:
    'flex w-full items-center justify-between rounded-md px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wider text-slate-500 transition-colors hover:bg-slate-50 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-slate-900/70 dark:hover:text-slate-200',
  ...
  sidebarBody: 'space-y-6 py-5 pl-4 pr-5',
  sidebarSection: 'space-y-2',
  accountSelectTrigger:
    'min-h-[var(--wa-account-select-h)] rounded-lg border-slate-200 bg-white px-3 py-2 pr-10 text-sm shadow-sm outline-none focus-visible:border-emerald-500 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-emerald-100 data-[state=open]:border-emerald-500 data-[state=open]:ring-2 data-[state=open]:ring-inset data-[state=open]:ring-emerald-100 dark:border-slate-800 dark:bg-slate-900',
  accountRow: 'flex min-w-0 flex-1 items-center gap-3 overflow-hidden',
  accountIcon:
    'flex h-[var(--wa-account-icon)] w-[var(--wa-account-icon)] shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400',
  // Filter item button (icon + label + count), with hover; pass active separately.
  filterItem:
    'grid min-h-10 w-full grid-cols-[20px_minmax(0,1fr)_auto] items-center gap-2 rounded-md px-2.5 py-2 text-sm transition-colors',
  filterItemIdle:
    'text-slate-700 hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-slate-900/70',
  filterItemActive:
    'bg-emerald-50 text-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-100',
  iconButton:
    'flex h-[var(--wa-icon-btn)] w-[var(--wa-icon-btn)] shrink-0 items-center justify-center rounded-md text-slate-600 transition-colors hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50 dark:text-slate-300 dark:hover:bg-slate-900',
  iconButtonSm:
    'flex h-[var(--wa-icon-btn-sm)] w-[var(--wa-icon-btn-sm)] shrink-0 items-center justify-center rounded-md text-slate-600 transition-colors hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50 dark:text-slate-300 dark:hover:bg-slate-900',
  avatarRow:
    'flex h-[var(--wa-avatar-row)] w-[var(--wa-avatar-row)] shrink-0 items-center justify-center overflow-hidden rounded-full bg-emerald-50 text-sm font-semibold text-slate-800 dark:bg-emerald-950/30 dark:text-emerald-100',
  avatarHeader:
    'flex h-[var(--wa-avatar-header)] w-[var(--wa-avatar-header)] shrink-0 items-center justify-center overflow-hidden rounded-full bg-emerald-50 text-sm font-semibold text-slate-800 dark:bg-emerald-950/30 dark:text-emerald-100',
  statGrid:
    'grid grid-cols-3 gap-2 border-b border-slate-200 bg-white px-3 py-2 dark:border-slate-800 dark:bg-slate-950',
  statButton:
    'group grid min-h-[var(--wa-stat-h)] min-w-0 grid-cols-[24px_minmax(0,1fr)_auto] items-center gap-2 rounded-md px-2 text-left transition-colors',
  statButtonIdle:
    'text-slate-500 hover:bg-slate-50 dark:text-slate-400 dark:hover:bg-slate-900/70',
  statButtonAccentActive:
    'bg-emerald-50 text-emerald-800 ring-1 ring-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-100 dark:ring-emerald-900/70',
  ...
  statCount:
    'shrink-0 text-[14px] font-semibold leading-none text-emerald-700 dark:text-emerald-200',
  statCountDanger:
    'shrink-0 text-[14px] font-semibold leading-none text-rose-600 dark:text-rose-200',
  listHeaderBody: 'space-y-3 bg-white px-3 pb-3 pt-3 dark:bg-slate-950',
  searchField:
    'h-11 rounded-lg border-slate-200/90 bg-white pl-10 pr-3 text-sm shadow-none transition-colors placeholder:text-slate-400 focus-visible:border-emerald-500 focus-visible:ring-2 focus-visible:ring-emerald-100 dark:border-slate-800 dark:bg-slate-950 dark:placeholder:text-slate-500',
  searchIcon:
  ...
  messageStack:
    'flex w-full min-w-0 flex-col gap-[var(--wa-chat-stack-gap)] overflow-x-hidden',
  messageBubbleBase:
  ...
  headerTextButton:
    'flex h-8 shrink-0 items-center gap-1 rounded-md border border-slate-200 px-2.5 text-xs font-medium text-slate-600 transition-colors hover:bg-slate-50 disabled:cursor-wait disabled:opacity-70 dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-900',
  composerDock:
    'flex-none border-t border-slate-200 bg-white px-4 py-[var(--wa-composer-pad)] dark:border-slate-800 dark:bg-slate-950',
  composerShell:
    'flex min-w-0 items-end gap-2 rounded-lg border border-emerald-600 bg-white p-2 shadow-sm focus-within:ring-2 focus-within:ring-emerald-100 dark:bg-slate-950',
};
